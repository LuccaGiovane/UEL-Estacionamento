...
class App:
    def __init__(self, middleware_ports=None):
        print("Iniciando App...")  # Mensagem de debug para verificar a inicialização
        self.middleware_ip = "127.0.0.1"  # IP padrão localhost
    ...
    def activate_station(self, station_number):
        ...
        try:
            ...
            with socket.create_connection((self.middleware_ip, port), timeout=10) as sock:
                message = "AE"
    ...
    def handle_input(self):
        while True:
            command = input(
                "Digite um comando (AE <número> ou RV <carro_id> <número> ou LV <carro_id> <número> ou V <número>): ").strip()
            parts = command.split()
            ...
            if len(parts) == 2 and parts[0] == "AE":
                ...
                print(
                    "Comando inválido. Tente AE <número_da_estação>, RV <carro_id> <número_da_estação>, LV <carro_id> <número_da_estação> ou V <número_da_estação>.")


if __name__ == "__main__":
    print("App iniciado. Aguardando comandos...")  # Mensagem de debug para ver se o script está inicializando
